...
class TargetOffsetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with torch.no_grad():
            image_embedding = self.model.encode_image(data)
        adjusted_target = target + self.offset
        return image_embedding, adjusted_target
...
def main(data_flag, output_root, num_epochs, gpu_ids, batch_size, download, model_flag, resize, as_rgb, model_path, run):
    ...
    MODEL_PATH = "/home/amunier/medicalAI/bestModel/best_model.pth"
    datasets_2D = [
    ('pathmnist', 'PathMNIST'),
...
# Concatenate datasets with adjusted target labels
    ...
    for _, dataset in datasets_2D:
        ...
        adjusted_train_dataset = TargetOffsetDataset(DataClass(split='train', transform=data_transform, download=download, as_rgb=as_rgb), model=clip_model,offset=offset)
        adjusted_val_dataset = TargetOffsetDataset(DataClass(split='val', transform=data_transform, download=download, as_rgb=as_rgb), model=clip_model, offset=offset)
        adjusted_test_dataset = TargetOffsetDataset(DataClass(split='test', transform=data_transform, download=download, as_rgb=as_rgb), model=clip_model, offset=offset)
